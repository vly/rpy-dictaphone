...
Live presentations audio recording is a well known problem for organisers, speakers and attendees alike. While there are a significant number of solutions available, factors such as cost, complexity and portability often limit widespread adoption. While existing services such as Echo360 and Lectopia work very well in recording in a lecture theatre environment, ad-hoc deployment in smaller venues can be costly and logistically difficult to achieve.
...
The focus of the project has been to deliver an easy-to-use, fast to setup, and cost-effective solution for live presentation recording. The following summary provides an overview of the core goals and projective we have strived towards over the course of the product development. The priority figure is the value on a scale of 1 to 5 (low to high respectively) that the team have placed on the goal/objective (particularly during the initial prototype phase).


\begin{description}

  \item[Cost-effectiveness (Priority 5)] \hfill \\
      One of the main factors that determines the viability of the end-product is the overall cost effectiveness. The aim was to provide a sub-\$200 product in a self-assembly state.
  \item[Ease of use (Priority 4)] \hfill \\
      The product had to be easy to use and operate by end-users. This includes setup, operation and troubleshooting stages of use. Some technical knowledge would be required for the current form of the product.
  \item[Semi-portability (Priority 3)] \hfill \\
      As the project aimed to primarily cater for ad-hoc recording scenarios, portability played an important part of the overall system design. This takes form in complexity of assembly, size and weight of the finished product, the power usage and requirement, as well as the storage capacity of the recording functionality.
...
We were fortunate to have formed a very focused team with complimentary skill sets. The feedback we received was very much a reflection of our hard work and determination. Overall we were very proud of what we achieved.


\subsection{Self Assessment}

From the start we knew this would be an ambitious project. We wanted to build something impressive, useful and which would exceed the learning objectives. On reflection we over-estimated what we could achieve in the time frame, however the positive feedback from what we did accomplish made us regard this project as a success.
...
During the lab demonstration we were asked the question "What mark do you think you deserve?" Our response was "HD" and we still stand by this answer. We treated all aspects of the project seriously and put in the hours needed to achieve the end solution. We delivered each milestone on time and did not compromise on the quality of our work.


At the end of this project there were a number of design changes we would make during the production of a second prototype. These include:

\begin{itemize}
    \item \textbf{Stronger servos:} In terms of the servos used for the project, they were sufficient in supporting the weight of the USB camera, however they struggled with the extra weight introduced by the shotgun microphone, cables, component mount and clamps. Spending an additional 20\% more money would have provided better servos which also come with standard metal gear kits.

    \item \textbf{Mounts and clamps:} The team had gone through various versions of camera and microphone mount prototypes before ending up with the final one used during the presentation. Although solid and fully adjustable, weight was a major tradeoff. The central shaft also reduced the vertical movement by approximately 30 degrees. A dedicated pan/swing mounting assembly as those used for DSLR/movie camera mounts would have provided greater range and smoothness of movements.

    \item \textbf{Camera:} Although the USB camera used for the project ended up working quite well, the resource constraints of the \rpi\xspace forced the team to optimise efficiency for face detection to work in real-time. These included significantly reducing the frame capture size and number of features detected. A camera with greater hardware-level control over the output (e.g. monochrome capture mode) would help to further reduce the resource overhead. The native \rpi\xspace camera module appeared to be a good candidate, however the current drivers and documentation are not mature enough for achieving the required results in this instance.

    \item \textbf{Power source:} Having gone through a number of issues getting a consistent external power source to drive the servos via the USB servo controller, and trying to manage the "cable spaghetti" caused by all the components; a single power source to drive all the parts of this project would have go a long way to resolve these issues. External laptop battery packs, although fairly heavy, do provide consistent power to run the project for at least six hours. The added bonus, is the size of such a battery pack could also serve as a base for the servo assembly.
\end{itemize}
...
In preparation for the  demonstration we developed a short slide show, wrote a speech and summary handout. All of these are available in the appendix. We also created a video of the working product in case we encountered technical issues during live demo. 
...
The reviews from our classmates during the presentation were very positive. There has been encouragement from a number of peers as well as our lab assistant to continue development of this project towards a commercially viable state. Of the 23 peer review forms we received, no one suggested a grade lower than a High Distinction, which was also very encouraging. 

Some suggestions offered by the peer feedback forms as well as verbally included:

\begin{itemize}
    \item Demonstrate the inner workings of the \rpis\xspace (a behind-the-scenes view of sorts) through a streaming debug activity to an external monitor.
...
Having never worked with a \rpi\xspace before, I found the project quite enlightening, particularly in understanding the lower-level hardware interfacing through GPIO pins. 
...
The work involving the servos and the \textit{Pololu} servo controller was also the first time I had a chance to prototype a hardware device library from API documentation, which has subsequently greatly increased my confidence in own abilities.

Lastly, being required to work on a resource constrained device (\rpi) has really highlighted the value of measurement and optimisation of system performance, which provided real-world context a lot of the topics covered during our lectures.

\subsubsection{Alfred}

During the lab, I learnt how to use linux command, and how to use cross-compiler to build a linux image.

I used a lot multi-threading programming, as a result of that I am now quite familiar with Synchronisation in a linux way. In addition, the face detection had requirement on memory management, so I spent a long time to investigate how to implement this in C++. Also, the message dispatch helped me understand about how to schedule jobs.
...
I had also never written an interface to device hardware before. It was very hard to debug but very satisfying once it was working. This also gave me the opportunity to expand my C and C++ experience.

The face detection was also very fun to work with. It was surprisingly easy to program and it has given me ideas for other potential projects where I could use this.
...
With the face detection (computer vision) generating major CPU load when running on the \rpi, the team had gone through a significant amount of iterations of the functionality in order to achieve real-time response to the environment.
...
Debugging and performance has been used extensively over the course of the project for both hardware and software components. For Python, Pudb was the debugger of choice. For C/C++ Instruments and LLDB/GDB were utilised. On the hardware side, the servo controller built-in error-checking was utilised for all communication to ensure hardware errors were caught and dealt with.
...
Working with the \rpi, our team encountered significant challenges in producing a functional project prototype while having so little processing power available at our disposal.

Significant amount of trial and error was carried out in optimising performance of face tracking functionality of the project (as can be observed by the GitHub repository history). 

The subject's coordinates derived from face-tracking functionality were then used to tilt and pan the assembly via two servos at the base of the camera and microphone combination. Initial experimentation with control of servos via \rpis GPIO pins were shown to be too inaccurate, and resulting servo movement too jagged to be used due to CPU interrupts effecting the clock cycle count. The chosen solution was to acquire an external hardware servo controller, the \textit{Pololu Maestro}. Running into issues with available libraries; our team put together a control library to communicate with the controller over the virtual  serial port. Prototyped first in Python using the device and protocol documentation, it was then ported to C for improved performance. The library has subsequently been open-sourced on GitHub.

The audio-recording functionality was moved to a secondary \rpi\xspace unit to maximise the recorded audio quality which was otherwise impacted by the face-tracking overhead.

Communication between the two \rpis\xspace was done over the network interface, with a purpose-built client/server daemon that managed the activation and deactivation of system elements.

\subsubsection{Learning Objective 3}

The project team put an emphasis on tracking ongoing development of project functions. As part of that, system and communication diagrams were created to detail and maintain visibility of the connection points of individual project sections to each other.
 
Some of the diagrams and descriptions that can be found throughout their relevant sections of this portfolio include:
...
\item \textbf{Processes and interprocess communication:} Having ended up with a solution involving multiple \rpis, the intercommunication channel between them, as well as the client-server communication functionality which runs over it are detailed and represented in a series of diagrams.
...
\item \textbf{Preemptive and non-preemptive scheduling:} Scheduling became particularly important during investigation into optimal audio recording setup. The original system design utilised only a single \rpi\xspace unit, requiring minor interrupts during audio-recording stage. Due to the system overhead of face-detection functionality, a design decision was made to move the audio recording to a separate \rpi. For optimal audio recording performance, kernel 3.8.13 recompiled with the real-time kernel patch enabling full preemption. Additionally, the audio recording process was set to -20 nice value.
 
\item \textbf{Memory hierarchy, management and cost-performance trade-offs:} Significant issues were encountered with getting the face-detection functionality to a perceived real-time state. Substantial evaluation and analysis of potential solutions/configurations and trade offs have been carried out to achieve the prototype state demonstrated in the lab. The topic is further expanded on in later sections.
 
\item \textbf{File system design and implementation:} For audio recording, a number of optimisations were made to improve the quality of the end-recordings. Some of the optimisation included mounting the file system with noatime flag to turn off Last Access Time (LAT) recording on file reads, decreasing the "swappiness" value to  introduce a greater delay to storage writes. 
...
Due to our time-poor circumstance, we relied heavily on version control, GitHub and regular discussion to ensure we were on target with all of our tasks. As our development logs (see appendix) demonstrate, we worked on the project heavily each week and also held meetings to work together on fixing or integrating hardware.
...
Finally, the time constraint coupled with the requirement of using a \rpi, meant we were also constrained also by our collective skill set. We were not familiar with OpenGL ES, but perhaps if we had had more time we could have dedicated time to learning the technology and porting the extensive OpenCV library to work on the \rpi\xspace GPU. 


\section{Development Methodology}

Our approach to developing the project was to divide it into manageable components and assign each part to a individual group member. At the start of each week we would communicate what had to be achieved to keep the project on track. 

A lot of care was taken to ensure that the weekly deliverables did not have flow on effects if they were not completed. This was so that time consuming or problematic components did not de-rail other members work if it was not finished on time. We have added a gantt chart to the appendix that demonstrates the lack of dependencies between each week.

\subsection{Programming languages}

From the outset our team decided that we wanted the system to run as efficiently as possible. Therefore it was an obvious choice to use C and C++ as the base language for the software. However, as we were experimenting with technology and techniques we had not used before, we used Python as a way to quickly prototype functionality. Once we were satisfied with the the prototype we would then port the code to C and C++.

Below is the example code we developed for interacting with the PIR-sensor. It was particularly important to prototype because it also involved determining if we had wired it correctly to the \rpi.
...
Each team member had different preferences of IDE and development tools, therefore a variety of tools were used.


\subsubsection{Vim}
    When working off the \rpi, we would occasionally need to tweak some code or write something experimental. Vim was an ideal tool for this, because we would be connected over SSH and it would allow us to use regular text editor features (e.g. syntax highlighting). As a long time C++ programmer, Alfred preferred to use Vim on his local machine as well.
...
Throughout the project there were an array of issues we encountered. Below is a summary of some of the significant difficulties we faced.
...
A number of issues were encountered while working on the audio component of the project. The general challenges included:
\begin{itemize}
\item Capture clean audio of tracked target.
\item Store audio in a more manageable format for archiving (e.g. mp3).
\item Process audio for audio-to-text conversion (stage 4 objective).
\end{itemize}

The \rpi\xspace was never built with audio-capturing as a use-case, for which the lack of audio-in stands a testament. An external USB audio card with a line-in was selected to provide the functionality.
 
A major issue encountered with with the above setup was the notable level of noise artefacts introduced by the \rpi\xspace  during recording. Researching the issue, a solution involving forcing the USB controller to USB1 speeds and physically distancing the external sound card from the actual  \rpi\xspace unit has vastly improved the audio-quality.
 
Another notable issue arose when working on audio-storage and format conversion functionality which was challenging given the \rpis\xspace  processing power.
A number of solutions were explored before selecting the Shine fixed-point MP3 encoding library for the end-prototype.
...
The system is comprised of two \rpis, networked together. The first device (or Server) is for subject tracking. It is connected to a USB camera, PIR-sensor and servo controller. The second device (or Client) is for audio recording. It is connected to a directional microphone. Figure \ref{fig:hardware} is an illustration of the complete hardware design.

\begin{figure}
\centering
\includegraphics[width=\textwidth]{graphs/hardware.pdf}
\caption{Complete system hardware.}
\label{fig:hardware}
\end{figure}

At boot time the Server \rpi\xspace initialises the master service, and awaits a client to connect. Once a connection has been established, the PIR sensor is activated and awaits movement in the direction of the speaker. One movement is detected, the face-tracking functionality is activated. As soon as a lock has been established on the speaker, servo pan and tilt functionality is engaged, and the Client RPi is notified to begin audio recording.

If speaker tracker functionality loses track of the target, it enters search mode where it pans from the last known coordinates until the target lock is reacquired. Failing that, it times out and returns to PIR listening state, notifying the Client \rpi\xspace to stop recording. The Client \rpi\xspace at that point, uses the downtime to begin conversion of recorded audio into the final archive format (MP3 mono) and removes original WAV file.

\subsection{Data Design}

Data storage on the  \rpi\xspace is severely limited by the size of the SD card. Although for most applications / use cases for the  \rpi\xspace a typical SD card is sufficient, the long term prospects for usage of our project platform is effected by the audio storage issue.
 
To address this issue, we have investigated various audio formats in search of a balance between audio quality, end file size, and usability for other project functions such as voice-to-text conversion. Some audio formats considered included WAV (base format), WMA voice, and MP3. Some voice recording specific codecs such as DSP Truespeech, although look great in specification, have no open source libraries available to use for this project.
 
For the prototype recording functionality archived audio state, we chose MP3 at 11.5kHz, 16kbps mono as this provided approximately a 5mb/hour file size which is a huge improvement over WAV which clocked in at approximately 600mb/hour.
...
\begin{description}
  ...
  \item[Communication - TCP vs UDP] \hfill \\
      We could have used either TCP or UDP for the inter \rpi\xspace communication. Since we only needed to communicate between two devices which would have a reduced rate of packet loss, we chose UDP. For our project a loss packet would not greatly effect the solution, so this also allowed us to avoid the complexity of managing failed deliveries.
  ...
  \item[Thread communication - Pipeline vs Shared memory] \hfill \\
      We chose to use shared memory for inter-thread communication. This was because shared memory provided better communication performance. Due to the number of threads we were using it also reduced the complexity needed to maintain the solution over something like a pipeline.

  \item[Memory - Instance every copy vs Copy on write] \hfill \\
      There was a lot of message data sent within the solution which can be accessed by all components. Instead of each component creating their own copy of a message instance, a copy-on-write method was adopted to avoid wasted object duplication.
      
  \item[Message scheduling - FCFS vs Complex algorithm] \hfill \\
      Every message in the system had the same priority, so first come first served was the best algorithm to implement and maintain for message scheduling. 
...
As the communication between server and client, we encode the message data into an array that uses `\textbackslash0' at the end. A message array can also be encoded into a message.The code of message serialization is shown as below:
...
// Based off: http://www.pololu.com/docs/0J40/5.h.1
...
Testing our system was both hard and slow. Due to the real world contextual behaviour of our project, our program did not lend itself to the traditional unit testing pattern.
...
The hardest part about experimenting with our system to improve performance was that we had to ensure it still responded as design. For example, some code changes would improve the CPU usage but decrease subject detection rate. These things were hard to measure and record.
...
All of our team members have enjoyed the project immensely. At times, it felt as though we had bitten off more than we could chew in regards to the scope, however this resulted in all of us going outside of our comfort zone and work together to achieve the prototype we presented in Week 10. This helped to realise practical, real-world applications to the material covered during the OSP lectures, and expand our horizons on what it takes to deliver a real-world solution when faced with a never-ending list of constraint.

From both our peers and the lab tutor there was a unanimous suggestion to continue development of this project towards a more commercially-viable state, which has provided a huge boost of confidence for the team members.
