...
A number of issues were encountered while working on the audio component of the project.
 
The general challenges included:
\begin{itemize}
...
The Raspberry Pi was never built with audio-capturing as a use-case, for which the lack of audio-in stands a testament. An external USB audio card with a line-in was selected to provide the functionality.
 
A major issue encountered with with the above setup was the notable level of noise artefacts introduced by the Pi during recording. Researching the issue, a solution involving forcing the USB controller to usb1 speeds and physically distancing the external soundcard from the actual RPi unit has vastly improved the audio-quality.
 
Another notable issue arose when working on audio-storage and format conversion functionality which was challenging given the RPi's processing power.
